...
本文件是 `recommendation-server` **内部 gRPC / proto 契约**的单一交付说明：`gateway` 或其他内部消费者实现集成时，**无需再读** `data-model.md` 即可理解场景、上下文、过滤、策略、条目、分页与解释载荷。

对外客户端 HTTP/JSON 由 `gateway` 映射；共享结果字段语义以 [共享契约规则](../../../.cursor/rules/shared-contracts.mdc) 为准。JSON 侧字段名使用 **snake_case**（如 `recommendation_id`、`guide_card_id`、`next_cursor`）。
...
- 若后续需要“纯图文/纯专题”推荐结果，应新增场景约束并同步更新 `.cursor/rules/shared-contracts.mdc` 与 gateway 映射
...
错误码区间与 `.cursor/rules/shared-contracts.mdc` 对齐：**通用 `10xxx`**、**鉴权 `20xxx`**、**本域推荐 `40xxx`**、**系统 `90xxx`**。业务层与 gRPC `details` 应对齐下列 **整数码**（与 proto 枚举 `RecommendationErrorCode` 取值一致，详见 `proto/recommendation_server_models.proto`）。同一语义跨版本保持同值，域内仅追加。
...
| `20001` | `AUTH_REQUIRED` | 鉴权 | 该 `scene` `requires_auth=true`（见 data-model §9.1）但当前为访客态 | 401 |
...
- `40003` 是**降级成功**而非失败：`success=true`、`data` 含降级后的 items，`code=40003` 经 gateway 放入 `warnings`（见共享契约「响应信封」`warnings` 仅在 `success===true` 出现）；客户端照常渲染，可选择弱化个性化解释。
...
- **多数读接口匿名 + 登录态透传**：`QueryRecommendations`、`GetPopularRecommendations`、`ExplainRecommendations` 默认**允许匿名**（仅 `session_id`）；`context.user_id`/`signal_bundle_ref` 由 gateway 在登录态下透传，用于个性化。是否强制登录由 `recommendation_scene_config.requires_auth` 决定（v1 默认 `false`，即匿名可用）。
- **身份单一来源**：主体身份只来自 gateway 透传，不接受请求体内 `acting_user_id` 等作为身份来源（对齐共享契约「鉴权与用户态」）。
- **无运营写接口**：本域 v1 不暴露面向 C 端的写 RPC；场景配置/策略发布走内部运营链路，角色校验在 `gateway`/运营后台完成，本域记录 `created_by/updated_by` 审计列（见 data-model §9.1/§9.2）。

---

## 9. 版本管理

- 对外 API 版本由 `gateway` 控制；**本域**以目录 `proto` 与包 `simple_living.recommendation_server` 为边界。
- 破坏性变更须同步：本文、`.cursor/rules/shared-contracts.mdc` 中相关字段、`gateway` 映射与 `changelog.md`（若仓库要求）。
