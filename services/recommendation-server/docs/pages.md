# recommendation-server — Page and surface mapping (v1)

Maps **product surfaces** to **scenes**, expected **internal recommendation APIs**, and **ownership** for parallel work. Clients consume public JSON from `gateway`; this file describes the recommendation-server side of the mapping. Full card payloads are loaded from `platform/backoffice-backend` unless BFF denormalization is explicitly agreed.
...
| UI block | Scene | API | Notes |
|----------|--------|-----|--------|
| Category ranked list | `theme_feed` | `QueryRecommendations` | Use `filters.theme_id` from `platform/backoffice-backend` taxonomy |
...
**Dependency**: Category ids and tree come from `platform/backoffice-backend`; recommendation-server does not define taxonomy.
...
| Behavior | Handling |
|----------|----------|
| Empty state | 默认返回成功结果 + `items: []`；是否降级到 platform/backoffice-backend 兜底由 gateway 决定 |
