...
- **分区策略（v1 单节点 lab）**：默认 1 分区/topic；生产按吞吐与并行消费需求提分区数（提分区数不可回退，规划时预留）。
...
- **容量假设（v1 单节点 lab）**：低吞吐事件流（发布/点击/转化），单分区可承载；节点 4096MB 内存约束下控制保留期与消息体积。
...
- HA：**v1 单节点 + 数据卷持久化**，多 broker/副本与跨可用区**范围外**，待生产阶段引入。
...
> 生产级可用性 SLA 与多 broker 目标在 HA 引入后单独定义，v1 明确范围外。
