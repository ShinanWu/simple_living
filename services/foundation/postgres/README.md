...
- **不负责**：业务规则与 schema 定义本身——各表结构、索引、迁移 SQL 由对应服务在 `services/<service>/migrations/` 与 `api.md` 拥有；本服务只提供运行实例与统一迁移/备份机制。
...
业务服务通过 `-pg_conninfo` flag 或等价 env 接收完整连接串（见各域 `README.md` 本地开发节）。
...
- **迁移回滚**：采用「只前滚」策略，迁移文件向后兼容、不写破坏性变更；如确需回退数据，使用 §5 的备份恢复到迁移前快照（不提供自动 down-migration）。
...
- **数据保留**：本地 lab 备份保留策略由运维按磁盘容量手动清理 `.foundation-backups/`（建议保留最近 7 份）；生产保留周期与加密归档按合规单独配置（未纳入的自动归档/异地副本）。
- **PITR（时间点恢复）**：当前仅提供 `pg_dump` 逻辑备份的「全量快照恢复」；WAL 归档 + PITR 待多副本/生产阶段引入。
...
约束：迁移**向后兼容、只前滚**；不可改写已应用文件（应新增版本）；破坏性变更需在对应域 Git 提交/PR 说明并列出受影响消费方。

## 7. 容量、连接数与 HA

- **容量假设（单实例 lab）**：单库承载全部域数据，初期数据量在 GiB 级；节点默认 `4096MB` 内存、`80G` 磁盘（见 `start_nodes.sh`）。
- **连接数**：业务侧用连接池约束并发；默认未调高 `max_connections`（沿用镜像默认 100），各域连接池上限之和应留余量。超限时调大 `max_connections` 或收紧池大小。
- **HA / 持久化策略**：lab 为**单实例 + 数据卷持久化 + 逻辑备份恢复**；生产以 K8s StatefulSet + 流复制与自动故障转移为目标架构。

## 8. 安全与网络边界

- **网络边界**：仅内网/隧道可达——foundation 节点 PostgreSQL 端口只经 QEMU host-forward 暴露在本机回环与业务来宾的 `10.0.2.2`，**不对公网开放**。
- **认证**：口令认证；账号/口令来源见 §3，密钥不写明文。
- **传输**：lab 内网为明文连接（可信网络内）；生产跨网段须启用 TLS。
...
> 生产级可用性 SLA（如 99.9%）与多副本目标随 HA 架构落地单独定义。
