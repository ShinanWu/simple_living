...
- **范围**：终端经前置 `Nginx` 到 `gateway` 的 **HTTPS + JSON** 约定；本文件包含用户路由与页面聚合路由、请求/响应 `data` 形状、嵌套对象、枚举、错误码与到各域 RPC 的映射说明。
- **路径前缀**：对外路由统一为 `/api/v2/...`，与内部 `simple_living.user_server` / `simple_living.gateway.user` proto 对齐。
...
### 5.12 `guide_card`（导购卡片）

可被推荐/列表/详情消费的统一内容卡片（纯导购，无站内交易）。网关从 `ContentService/BatchGetGuideCards` 裁剪为下列对外形状。

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `guide_card_id` | string | 是 | 建议 UUID |
| `schema_version` | integer | 是 | 从 1 递增 |
| `title` | string | 是 | |
| `subtitle` | string | 否 | |
| `summary` | string | 否 | |
| `cover_media` | object | 否 | 见下 |
| `theme` | string | 是 | 枚举 `theme` |
| `tags` | string[] | 否 | |
| `published_at` | string | 是 | ISO 8601 UTC |
| `updated_at` | string | 是 | ISO 8601 UTC |
| `commercial_disclosure` | object | 是 | 见下 |
| `affiliate_context` | object | 否 | 见下 |
| `attributes` | object | 否 | 扩展 KV；键 `snake_case`，值仅 JSON 安全类型 |

`cover_media`：`type`（`image` \| `video_poster`）、`url`（HTTPS）、`width`、`height`。

`commercial_disclosure`：`is_commercial`（boolean）、`partner_id`（可选）、`disclosure_text_key`（可选，客户端文案 key）。

`affiliate_context`：`offer_id`、`channel_codes`（string[]）；客户端不得篡改回传。跳转 URL 不由卡片静态字段承载，由 `POST /api/v2/pages/redirect_prepare` 返回。

兼容性：只允许新增可选字段；`schema_version` 升级时的破坏性变更在 PR 说明。

### 5.13 `attribution`（跳转归因回显）

`redirect_prepare` 响应中的只读子集。完整 RPC 语义见 [tracking-server api.md](../tracking-server/api.md)。

| 字段 | 类型 | 说明 |
|------|------|------|
| `click_id` | string | 单次点击 ID |
| `trace_id` | string | 可选 |
| `session_id` | string | 可选 |
| `user_id` | string | 可选；访客勿填假值 |
| `channel_code` | string | 如 `pdd`、`douyin` |
| `client_platform` | string | |
| `client_version` | string | 可选 |
| `guide_card_id` | string | |
| `recommendation_id` | string | 可选 |
| `scene` | string | 可选 |
| `item_rank` | integer | 可选 |
...
| `theme` | `clothing`, `food`, `housing`, `transport` | Tab 默认顺序固定；UI 文案可本地化，请求参数必须用枚举值；仅允许向后兼容追加 |
...
客户端以 JSON **`code`** 为准。五位整数 `ABCDE`：`A` 为域大类，后四位为域内序号；同一语义保持同一数值；域内仅允许追加。顶层 `code` 表主因，`errors[].code` 表字段级细粒度。

| 区间 | 域 |
|------|-----|
| `0` | 成功 |
| `10000`–`10999` | 通用客户端 / 请求 / 网关入口（`10050`–`10099` 预留给 gateway 入口层） |
| `20000`–`20999` | 鉴权与会话（详见 [user-server api.md](../user-server/api.md)） |
| `30000`–`30999` | 内容与导购（详见 [platform api.md](../platform/api.md)） |
| `40000`–`40999` | 推荐（详见 [recommendation-server api.md](../recommendation-server/api.md)） |
| `50000`–`50999` | 联盟与跳转（详见 [tracking-server api.md](../tracking-server/api.md)） |
| `60000`–`60999` | 治理与合规（详见 [platform api.md](../platform/api.md)） |
| `90000`–`90999` | 系统 |
...
| 区间 | 来源域 | 典型码（语义见各域 `api.md`） |
|------|--------|------------------------------------------|
| `30000`–`30999` | platform/backoffice-backend（见 `../platform/api.md`） | `30001` 不存在、`30002` 已下架/不可见、`30003` 未发布 |
| `40000`–`40999` | recommendation-server（见 `../recommendation-server/api.md`） | `40001` 场景不支持、`40002` 推荐不可用、`40003` 已降级 |
| `50000`–`50999` | affiliate / tracking（见 `../tracking-server/api.md`、`../platform/api.md`） | `50001` 渠道不可用、`50002` 转链失败、`50003` 链接失效 |
| `60000`–`60999` | platform/backoffice-backend（见 `../platform/api.md`） | `60001` 合规拦截、`60002` 地域/策略限制 |
...
**禁止**：在网关引入与 `user_server_models.proto` / 本文件 **含义冲突** 的新字段语义。
...
多域只读拼屏的**可交付路由定义以本文件 §13 为准**。细粒度用户路由与 `GetMeSummary` 均可被页面聚合复用。部分依赖失败时可降级并附 `warnings`（仅 `success===true`），见 §8.1 `10053`/`10054`。
...
| `attribution` | object | 否 | 回显的归因字段子集，见 §5.13 |
...
| `theme` | string | 否 | 可选主题过滤；枚举见 §6 `theme` |
...
| `preferred_channel_code` | 用于组装 `affiliate_context_ref` 或路由策略 | 若上游传入则必须透传到组链策略；未传时由服务端默认策略选择渠道 |
...
以下示例使用真实对外 JSON 形状，便于前端、测试与其他 Agent 直接联调。响应信封遵循 §3；时间字段使用 ISO 8601 UTC 字符串。文档示例 ID 见附录 C。
...
```json
{"success":true,"code":0,"message":"ok","data":{"consent":{"personalization_allowed":true,"consent_version":"2026-03-01","updated_at":"2026-03-28T12:00:00Z"}},"meta":{"request_id":"req_15","server_time_ms":1774699800000}}
```
...
{"consent":{"personalization_allowed":true,"consent_version":"2026-03-01","updated_at":"2026-03-28T12:00:00Z"}}
```

#### HTTP Response

```json
{"success":true,"code":0,"message":"ok","data":{"consent":{"personalization_allowed":true,"consent_version":"2026-03-01","updated_at":"2026-03-28T12:00:00Z"}},"meta":{"request_id":"req_16","server_time_ms":1774699800000}}
```
...
```json
{"success":true,"code":0,"message":"ok","data":{"profile":{"user_id":"user_01HSYQK5PZ4K4J9R2M8D","is_guest":false},"counts":{"favorites_count":0,"history_count":0},"consent":{"personalization_allowed":true,"consent_version":"2026-03-01","updated_at":"2026-03-28T12:00:00Z"}},"meta":{"request_id":"req_20","server_time_ms":1774699800000}}
```

## 附录 B. 典型错误响应示例

错误响应同样遵循 §3 信封：`code !== 0` ⇔ `success === false`；`data` 多为 `null`；客户端以 `code` 分支。
...
对外 JSON 时间用 ISO 8601 UTC；内部 `proto`/RPC 示例用 `textproto`，枚举写 proto 枚举名。
