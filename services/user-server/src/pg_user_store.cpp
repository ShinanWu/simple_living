...
std::string PgUserStore::AppIdFromContext(const RpcRequestContext& ctx) {
    return ctx.has_app_id() ? ctx.app_id() : std::string("");
}
...
void PgUserStore::IssueTokenPair(const IssueTokenPairRequest& req, IssueTokenPairResponse* resp) {
    ...
    std::string app_id = req.has_request_context() ? AppIdFromContext(req.request_context()) : std::string("");
    std::string sid = GenId("sid");
    std::string at = GenId("at");
    std::string rt = GenId("rt");
    const char* pv2[] = {sid.c_str(), user_id.c_str(), at.c_str(), rt.c_str(), app_id.c_str()};
    r = ExecParams(
        "INSERT INTO user_session (session_id, user_id, access_token, refresh_token, is_guest, app_id) "
        "VALUES ($1, $2, $3, $4, false, $5)",
        5, pv2, nullptr, nullptr);
...
void PgUserStore::EnsureGuestSession(const EnsureGuestSessionRequest& req, EnsureGuestSessionResponse* resp) {
    ...
    const char* p2[] = {sid.c_str(), user_id.c_str(), at.c_str(), rt.c_str(), app_id.c_str()};
    r = ExecParams(
        "INSERT INTO user_session (session_id, user_id, access_token, refresh_token, is_guest, app_id) "
        "VALUES ($1, $2, $3, $4, true, $5)",
        5, p2, nullptr, nullptr);
    ClearRes(r);
...
void PgUserStore::GetPreferencesUnlocked(const GetPreferencesRequest& req, GetPreferencesResponse* resp) {
    const char* pv[] = {"", req.user_id().c_str()};
    PGresult* r =
        ExecParams("SELECT prefs_hex FROM user_preferences_blob WHERE app_id = $1 AND user_id = $2", 2, pv, nullptr, nullptr);
...
void PgUserStore::UpdatePreferences(const UpdatePreferencesRequest& req, UpdatePreferencesResponse* resp) {
    ...
    PGresult* r = ExecParams(
        "INSERT INTO user_preferences_blob (app_id, user_id, prefs_hex) VALUES ($1, $2, $3) "
        "ON CONFLICT (app_id, user_id) DO UPDATE SET prefs_hex = EXCLUDED.prefs_hex",
...
void PgUserStore::ListFavorites(const ListFavoritesRequest& req, ListFavoritesResponse* resp) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (!PQgetisnull(r, i, 2)) {
            it->set_content_type(static_cast<ContentRefType>(std::atoi(PQgetvalue(r, i, 2))));
        }
    }
    ClearRes(r);
}
...
void PgUserStore::AddFavorite(const AddFavoriteRequest& req, AddFavoriteResponse* resp) {
    std::lock_guard<std::mutex> lock(mu_);
    std::string content_type_str = req.has_content_type()
        ? std::to_string(static_cast<int>(req.content_type()))
        : std::string("0");
    const char* pv[] = {"", req.user_id().c_str(), req.content_id().c_str()};
    PGresult* r = ExecParams(
        "SELECT favorite_id FROM user_favorite WHERE app_id = $1 AND user_id = $2 AND content_id = $3",
        3, pv, nullptr, nullptr);
    if (r && PQresultStatus(r) == PGRES_TUPLES_OK && PQntuples(r) > 0) {
    ...
    const char* pv2[] = {fid.c_str(), req.user_id().c_str(), req.content_id().c_str(),
                         content_type_str.c_str(), ""};
    r = ExecParams(
        "INSERT INTO user_favorite (favorite_id, user_id, content_id, content_type, app_id) "
...
void PgUserStore::RemoveFavorite(const RemoveFavoriteRequest& req, RemoveFavoriteResponse* resp) {
    ...
    if (!req.favorite_id().empty()) {
        ...
        r = ExecParams("DELETE FROM user_favorite WHERE user_id = $1 AND favorite_id = $2", 2, pv, nullptr,
                       nullptr);
    } else if (!req.content_id().empty()) {
        const char* pv[] = {"", req.user_id().c_str(), req.content_id().c_str()};
        r = ExecParams("DELETE FROM user_favorite WHERE app_id = $1 AND user_id = $2 AND content_id = $3",
                       3, pv, nullptr, nullptr);
    } else {
...
void PgUserStore::ListHistory(const ListHistoryRequest& req, ListHistoryResponse* resp) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (!PQgetisnull(r, i, 5)) {
            it->set_source_surface(static_cast<HistorySourceSurface>(std::atoi(PQgetvalue(r, i, 5))));
        }
    }
    ClearRes(r);
}
...
void PgUserStore::RecordHistoryEvent(const RecordHistoryEventRequest& req, RecordHistoryEventResponse* resp) {
    ...
    std::string cid = req.has_content_ref() ? req.content_ref().content_id() : "";
    std::string ctype = req.has_content_ref() && req.content_ref().has_type()
        ? std::to_string(static_cast<int>(req.content_ref().type()))
        : std::string("0");
    std::string ssurf = req.has_source_surface()
        ? std::to_string(static_cast<int>(req.source_surface()))
        : std::string("0");

    int64_t occurred_secs = 0;
    if (req.has_occurred_at()) {
        occurred_secs = req.occurred_at().seconds();
    } else {
        occurred_secs = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
    }
    std::string occurred_str = std::to_string(occurred_secs);

    const char* pv[] = {ok.c_str(), cid.c_str(), ctype.c_str(), "",
                        occurred_str.c_str(), occurred_str.c_str(), ssurf.c_str()};
    PGresult* r = ExecParams(
        "INSERT INTO user_history (owner_key, content_id, content_type, app_id, "
        "last_seen_at, first_seen_at, impression_count, source_surface) "
        "VALUES ($1, $2, $3::int, $4, "
        "to_timestamp($5::bigint), to_timestamp($6::bigint), 1, $7::int) "
        "ON CONFLICT (owner_key, content_id) DO UPDATE SET "
        "last_seen_at = GREATEST(user_history.last_seen_at, to_timestamp($5::bigint)), "
        "first_seen_at = LEAST(user_history.first_seen_at, to_timestamp($6::bigint)), "
        "impression_count = user_history.impression_count + 1, "
        "source_surface = $7::int",
        7, pv, nullptr, nullptr);
    char* nt = r ? PQcmdTuples(r) : nullptr;
    int nr = nt ? std::atoi(nt) : 0;
    ClearRes(r);
    if (nr > 0) {
...
void PgUserStore::ClearHistory(const ClearHistoryRequest& req, ClearHistoryResponse* resp) {
    ...
    PGresult* r = nullptr;
    if (req.scope() == CLEAR_HISTORY_SCOPE_BEFORE_TIME && req.has_before_time()) {
        int64_t before_secs = req.before_time().seconds();
        std::string before_str = std::to_string(before_secs);
        const char* pv[] = {ok.c_str(), before_str.c_str()};
        r = ExecParams(
            "DELETE FROM user_history WHERE owner_key = $1 AND last_seen_at <= to_timestamp($2::bigint)",
            2, pv, nullptr, nullptr);
    } else {
        const char* pv[] = {ok.c_str()};
        r = ExecParams("DELETE FROM user_history WHERE owner_key = $1", 1, pv, nullptr, nullptr);
    }
    char* nt = r ? PQcmdTuples(r) : nullptr;
    int nr = nt ? std::atoi(nt) : 0;
    ClearRes(r);
...
void PgUserStore::SubmitFeedback(const SubmitFeedbackRequest& req, SubmitFeedbackResponse* resp) {
    ...
    PGresult* r = ExecParams(
        ...
        "client_request_id, app_id) VALUES ($1, $2, $3, $4::int, $5, $6, $7)",
        7, pv, nullptr, nullptr);
    ClearRes(r);
...
void PgUserStore::GetConsentUnlocked(const GetConsentRequest& req, GetConsentResponse* resp) {
    const char* pv[] = {"", req.user_id().c_str()};
    PGresult* r =
        ExecParams("SELECT consent_hex FROM user_consent_blob WHERE app_id = $1 AND user_id = $2", 2, pv, nullptr, nullptr);
    if (!r || PQresultStatus(r) != PGRES_TUPLES_OK || PQntuples(r) < 1 || PQgetisnull(r, 0, 0)) {
        ClearRes(r);
        return;
    }
...
void PgUserStore::UpdateConsent(const UpdateConsentRequest& req, UpdateConsentResponse* resp) {
    ...
    const char* pv[] = {"", req.user_id().c_str(), hex.c_str()};
    PGresult* r = ExecParams(
        "INSERT INTO user_consent_blob (app_id, user_id, consent_hex) VALUES ($1, $2, $3) "
        "ON CONFLICT (app_id, user_id) DO UPDATE SET consent_hex = EXCLUDED.consent_hex",
        3, pv, nullptr, nullptr);
    ClearRes(r);
...
void PgUserStore::GetSignalBundleRef(const GetSignalBundleRefRequest& req, GetSignalBundleRefResponse* resp) {
    ...
    PGresult* r2 = ExecParams(
        "INSERT INTO user_signal (user_key, signal_ref, bundle_version, app_id) VALUES ($1, $2, $3, $4) "
