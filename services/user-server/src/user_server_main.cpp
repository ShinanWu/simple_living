// user-server brpc server — PostgreSQL (libpq) + Redis/Kafka per services/README.md.
...
DEFINE_int32(port, 9101, "TCP port for user-server brpc server");
...
class UserServerServiceImpl : public UserServerService {
    PgUserStore* store_;

public:
    explicit UserServerServiceImpl(PgUserStore* s) : store_(s) {}
    ...
    void HealthCheck(::google::protobuf::RpcController*,
        ...
        resp->set_status(pg_ok ? "SERVING" : "DEGRADED");
        auto* c = resp->add_components();
        c->set_name("primary-db");
        c->set_ok(pg_ok);
...
int main(int argc, char* argv[]) {
    ...
    simple_living::user_server::UserServerServiceImpl svc(&store);
    brpc::Server server;
    if (server.AddService(&svc, brpc::SERVER_DOESNT_OWN_SERVICE) != 0) {
        LOG(ERROR) << "Fail to add UserServerService";
        return 1;
    }

    brpc::ServerOptions options;
    if (server.Start(FLAGS_port, &options) != 0) {
        LOG(ERROR) << "Fail to start user-server server";
        return 1;
    }

    LOG(INFO) << "user-server server listening on port " << FLAGS_port;
