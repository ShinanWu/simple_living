...
刷新令牌无效/过期时，v1 同样返回 **gRPC `OK` + `valid = false`**，其余字段应为空。
...
`provider` 不是 proto 枚举，v1 作为**服务自管字符串码表**处理；调用方必须使用 `user-server` 已注册的登录提供方编码（推荐小写 ASCII，如 `wechat`、`apple`、`douyin`、`google`），未知值返回 `INVALID_ARGUMENT` / `10002`。
...
`GetSignalBundleRefRequest`：**oneof `subject`** `user_id` | `session_id`；`scene`。`scene` 为语义必填，使用稳定的 `snake_case` 场景字符串。v1 允许的 `scene` 值由接入应用在服务配置中注册；未注册的值返回 `INVALID_ARGUMENT`，并建议在网关侧映射为 `40001`。
...
面向下游服务的 v1 最小可依赖 JSON 键集合为：`scene`（string）、`subject_kind`（`user` / `guest`）、`theme_interests`（string array）、`content_filters`（object<string,string>）、`recent_content_ids`（string array，可为空）。新增键仅追加，既有键语义不得重写。
...
v1 约定：若请求中未携带 `app_id`，服务端使用默认应用配置；此行为仅用于向后兼容，新接入应用必须携带 `app_id`。

## 9. 幂等、乐观锁与冲突落定（v1 确定值）
...
> **统一约定**：所有“版本化整文档写”的陈旧写入/版本冲突一律返回业务码 **`10007`**（gRPC `ABORTED`），与 §6.2 和 `.cursor/rules/shared-contracts.mdc` 完全一致；本域不为该语义再造其他码。
