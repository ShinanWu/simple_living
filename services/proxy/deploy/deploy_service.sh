...
DOCKER_VOLUMES="-v ${TLS_HOST_DIR}:/etc/nginx/certs:Z -v ${ACME_HOST_DIR}:/var/www/certbot:Z"

REMOTE_DEPLOY_CMD="if sudo -n docker version >/dev/null 2>&1; then ctr='sudo -n docker'; elif sudo -n podman version >/dev/null 2>&1; then ctr='sudo -n podman'; elif command -v docker >/dev/null 2>&1; then ctr=docker; else ctr=podman; fi; sudo mkdir -p '${TLS_HOST_DIR}' '${ACME_HOST_DIR}'; (\$ctr rm -f simple-living-proxy >/dev/null 2>&1 || true); (\$ctr rm -f simple-living-nginx-proxy >/dev/null 2>&1 || true); \$ctr run --pull=never -d --name simple-living-proxy --restart unless-stopped ${DOCKER_PUBLISH_PORTS} ${DOCKER_VOLUMES} -e ENABLE_FRP='${ENABLE_FRP}' -e ENABLE_INGRESS_HTTPS='${ENABLE_INGRESS_HTTPS}' -e FRP_REMOTE_HTTPS_PORT='${FRP_REMOTE_HTTPS_PORT}' -e FRP_EMBEDDED='${FRP_EMBEDDED}' -e FRP_PROXY_TYPE='${FRP_PROXY_TYPE}' -e FRP_SERVER_ADDR='${FRP_SERVER_ADDR}' -e FRP_SERVER_PORT='${FRP_SERVER_PORT}' -e FRP_AUTH_TOKEN='${FRP_AUTH_TOKEN}' -e FRP_CUSTOM_DOMAIN='${FRP_CUSTOM_DOMAIN}' -e FRP_USER='${FRP_USER}' -e FRP_REMOTE_PORT='${FRP_REMOTE_PORT}' -e PROXY_GATEWAY_UPSTREAM_HOST='${PROXY_GATEWAY_UPSTREAM_HOST}' -e PROXY_BACKOFFICE_UPSTREAM_HOST='${PROXY_BACKOFFICE_UPSTREAM_HOST}' '${IMAGE_NAME}' >/dev/null && [ \$(\$ctr inspect -f '{{.State.Status}}' simple-living-proxy 2>/dev/null || echo unknown) = running ]"
