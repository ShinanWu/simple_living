...
- 业务字段语义、页面聚合、JSON↔proto 映射、鉴权与会话 → `gateway`（`../../gateway/`）。
- 业务路由真相（路径/版本/动作语义）→ `gateway`（`../../gateway/api.md`，详见 §6.2/§6.3）。
- 对外错误信封与公共 JSON 语义 → `services/gateway/api.md`。

> **无业务契约文档**：proxy 是入口基础服务，不拥有业务 `api.md` / `proto`；仅维护 `README.md` + `deploy/README.md` 即可。

---

## 服务等级目标（SLO）

仅承诺入口转发层目标；业务接口可用性/延迟以 gateway 为权威（`../../gateway/README.md`）。
...
- 所有业务路由的增删改只在 gateway 的 `services/gateway/api.md` 和代码中维护
