...
This document is the **delivery-ready** contract for `tracking-server` internal services. **`gateway`** maps public HTTPS + JSON to these RPCs; shared external semantics use **`snake_case`** and names such as `landing_url`, `click_id`, `guide_card_id`, `recommendation_id`, `scene` per `services/gateway/api.md`.
...
- Breaking changes → new package version; record in Git commit/PR description.
...
Connection strings, signing keys and partner secrets come from deploy-time secrets/env, never from the repo or request bodies (see [README.md](./README.md) §安全与密钥).
...
When `device_dedup_key` is omitted, the service does not create a global `(short_token, "")` dedup key. The request may still create a click fact, subject only to other anti-abuse or rate-control policies.
...
**No secrets** in this message. This section is the authoritative RPC-level mapping for tracking callers.
...
| `placement` | `string` | |
| `channel_code` | `string` | Stable outbound channel code, e.g. `pdd`, `douyin`; it defaults to affiliate `partner_id` unless an explicit alias mapping is documented upstream |
...
only promotes `sub_id_1..3` into tracking attribution snapshots. Additional affiliate sub slots may exist upstream but are not guaranteed to survive into this contract.
...
| `conversion_type` | `ConversionType` | yes | Affiliate normalized events use `CONVERSION_TYPE_ORDER_PAID` |
...
- `CommissionEventStatus.REVERSED` / `INVALID` must not be ingested as new positive conversion facts through this RPC path; they belong to reversal / adjustment handling outside `IngestConversion`
- conflicts between persisted click attribution and incoming affiliate correlation hints must not cause hard ingest failure on this path; tracking should trust its persisted snapshot, retain the incoming hints for audit, and continue ingest unless another validation rule fails
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `pagination` | `CursorPagination` | yes | `cursor`, `limit` per `services/gateway/api.md` |
| `user_id` | `string` | no | |
...
| `public_code` | `int32` | 5-digit public `code` (§9.2) carried for gateway mapping |
| `link_ref` | `string` | When relevant |
| `retryable` | `bool` | |

### 9.2 Public error-code mapping (gateway envelope)

Internal RPC returns a non-OK status; `gateway` maps it to the public envelope `code` per `services/gateway/api.md`. This domain uses the **affiliate & redirect `50xxx`** band, plus generic `10xxx` and system `90xxx`. Same semantic keeps the same number across versions; this band is append-only.
...
- **`ResolveRedirect` click dedup:** at most one primary click per `(short_token, device_dedup_key)`; a duplicate collapses to the existing `click_id`, not an error.
...
- Tracking does **not** assume any extra public affiliate RPC for redirect-time refresh. If the previously validated spec expires or needs refreshed token material, tracking must either re-run the same `affiliate_context_ref -> LinkGenerationInput -> ValidateLinkGenerationInput` path or rely on affiliate private adapter capability explicitly owned inside platform/backoffice-backend.
...
- Reversal / adjustment handling is explicitly outside the positive-conversion RPC surface documented here; implementers of the main affiliate->tracking path do not need to invent an extra RPC in this task.

### 11.2 **gateway**

- Owns HTTPS routes, envelopes, HTTP status codes, and JSON **`snake_case`** mapping.
- Public list responses place pagination under `data.pagination` per gateway `api.md` §4.

### 11.3 **contracts**

- Redirect / attribution field semantics: `services/gateway/api.md`.
- Pagination: `services/gateway/api.md`.
