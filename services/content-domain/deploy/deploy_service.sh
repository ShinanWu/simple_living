...
SERVICE_NAME="content-domain"
IMAGE_REPO="content-domain"
IMAGE_TAG="${IMAGE_TAG:-latest}"
BASE_IMAGE="${BASE_IMAGE:-debian:bookworm-slim}"
TARGET_HOST="${NODE_IP_CONTENT}"
SSH_PORT="${SSH_PORT:-22}"
TARGET_SSH_PORT="${NODE_SSH_PORT_CONTENT:-${SSH_PORT}}"
...
REMOTE_BUILD_CMD="cd '${QEMU_BUILD_WORKDIR}' && if [[ -f .bazelversion ]]; then export USE_BAZEL_VERSION=\$(tr -d '\n' < .bazelversion); fi; if command -v bazelisk >/dev/null 2>&1; then bazelisk build ${BAZEL_BUILD_ARGS:-} //services/content-domain:content_domain_server; elif command -v bazel >/dev/null 2>&1; then bazel build ${BAZEL_BUILD_ARGS:-} //services/content-domain:content_domain_server; else echo 'missing bazel/bazelisk'; exit 2; fi && build_ctx=\$(mktemp -d) && cp bazel-bin/services/content-domain/content_domain_server \${build_ctx}/server && cp services/content-domain/deploy/Dockerfile.cpp-service \${build_ctx}/Dockerfile.cpp-service && if command -v docker >/dev/null 2>&1; then ctr=docker; else ctr=podman; fi && \$ctr build --build-arg BASE_IMAGE='${BASE_IMAGE}' -f \${build_ctx}/Dockerfile.cpp-service -t '${IMAGE_NAME}' \${build_ctx} && rm -rf \${build_ctx}"
...
EXTRA_RUN_ARGS=""
REMOTE_DEPLOY_CMD="if sudo -n docker version >/dev/null 2>&1; then ctr='sudo -n docker'; elif sudo -n podman version >/dev/null 2>&1; then ctr='sudo -n podman'; elif command -v docker >/dev/null 2>&1; then ctr=docker; elif command -v podman >/dev/null 2>&1; then ctr=podman; else echo 'missing docker/podman'; exit 2; fi; (\$ctr rm -f 'simple-living-content-domain' >/dev/null 2>&1 || true) && \$ctr run --pull=never -d --name 'simple-living-content-domain' --restart unless-stopped -p '9102:9102' ${EXTRA_RUN_ARGS} '${IMAGE_NAME}' >/dev/null && [ \$(\$ctr inspect -f '{{.State.Status}}' 'simple-living-content-domain' 2>/dev/null || echo unknown) = running ]"
