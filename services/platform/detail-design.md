...
| 对外契约 | `gateway/` | [backoffice-gateway-api.md](./backoffice-gateway-api.md) |
| 写面后端 | `backoffice-backend/` | [api.md](./api.md)、[deploy/README.md](./backoffice-backend/deploy/README.md) |
...
**编辑**：`guide_card` 字段完整；`editorial_content` / `topic` / `ranking_list` 可选类型但无专属表单。
...
完整 JSON 定义见 [backoffice-gateway-api.md](./backoffice-gateway-api.md)。
...
模块 RPC 见 [api.md](./api.md)；实现见 `backoffice-backend/src/`。
...
| 规格 | 现状 |
|------|------|
| 四资源类型完整编辑 | 当前仅 `guide_card` 可创建；其余类型 UI 只读 |
| scheduled 定时发布 UI | 未实现 |
| 角色分权 RBAC | 单 token 鉴权，未按路径校验角色 |
| 后端权威审计落库 | UI 侧栏日志；服务端 outbox 部分覆盖 |
