...
契约权威：[backoffice-gateway-api.md](./backoffice-gateway-api.md)、[`services/gateway/api.md`](../../gateway/api.md)。
...
紧急不可见等风控在**可见性裁决**（`restricted` / `unpublished`）表达。
...
```
draft ──送审──> in_review ──治理通过──> approved ──显式发布──> published ──下架──> offline ──归档──> archived
  ^                │
  └──拒绝/补材──────┘
```
...
| 状态 | 含义 |
|------|------|
| `in_review` | 已送审，等待治理裁决 |
| `approved` | 治理已通过，待运营发布（**不是** C 端已上线） |

### 4.2 审核单（`review status`）

`pending` → `in_review` → `approved` / `rejected` / `needs_info` → `completed`。  
治理通过后 `content_status` 变为 `approved`（待发布），不自动变为 `published`。
...
**当前未纳入**：批量导入导出、定时发布 UI、四资源类型完整编辑表单、富文本编辑器、独立风控台、全资源类型 UI 与素材库/CDN 工作台（见 [detail-design.md](./detail-design.md) §6）。
