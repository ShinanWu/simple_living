...
host="${NODE_IP_RECOMMENDATION:-}"
if [[ -z "${host}" ]]; then
  echo "SKIP: NODE_IP_RECOMMENDATION not set"
else
  port="${NODE_SSH_PORT_RECOMMENDATION:-${SSH_PORT}}"
  echo "==> checking NODE_IP_RECOMMENDATION (${host}:${port})"
  ok=0
  for _ in $(seq 1 12); do
    if ssh "${SSH_OPTS[@]}" -p "${port}" -i "${SSH_KEY_PATH}" "${SSH_USER}@${host}" "((command -v docker >/dev/null 2>&1) || (command -v podman >/dev/null 2>&1)) && command -v systemctl >/dev/null 2>&1 && command -v curl >/dev/null 2>&1"; then ok=1; break; fi
    sleep 5
  ...
  if [[ "${ok}" -ne 1 ]]; then
    FAILED+=("NODE_IP_RECOMMENDATION(${host}:${port})")
  fi
fi
if [[ "${#FAILED[@]}" -gt 0 ]]; then
