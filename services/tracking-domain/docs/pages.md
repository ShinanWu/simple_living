# tracking-domain — Client surfaces & integration
...
4. **Mini-program constraints:** If outbound rules require opening external browser or specific APIs, follow platform capability flags from **affiliate-domain** responses (tracking consumes the outcome of assembly only).
...
**Critical:** Some platforms restrict raw redirects; **tracking-domain** remains authoritative that the **first network hit** to tracking is recorded — clients must not skip it.

## 5. Backend / batch agents

| Job | Integration |
|-----|-------------|
| Nightly reconciliation | Read `commissions` views; compare with **affiliate-domain** exports. |
...
- Visual design of CTAs (`content-domain` / design system).
- Which partners appear on a card (`affiliate-domain` + `content-domain`).
