...
sudo cp infra/ingress/frp/frps.toml.example /etc/frp/frps.toml
...
```bash
sudo mkdir -p /etc/frp
sudo cp infra/ingress/frp/frpc.toml.example /etc/frp/frpc.toml
...
```bash
sudo cp infra/ingress/nginx/gateway.conf.example /etc/nginx/conf.d/gateway.conf
sudo nginx -t
...
```bash
bash infra/ingress/scripts/check_ingress.sh https://<your_domain_or_ip>
```
