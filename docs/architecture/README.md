...
4. **统一入口**：所有终端通过前置 `Nginx` 进入网络层，再由 `gateway` 进入后端体系；`gateway` 是唯一业务入口与 BFF
...
┌────────────────────────────────────────────────────────────────────────────┐
│                              平台与治理层                                  │
│ GitLab / GitLab CI / ArgoCD / Bazel / K8s / Vault /                         │
...
- 服务与服务之间统一使用 `proto`
- `Nginx` 仅承担通用网络能力，不承载业务字段语义，不替代 `gateway` 的 BFF 职责
...
| **构建系统** | Bazel |
| **容器编排** | Kubernetes |
| **K8s 使用范围** | 仅容器编排与服务注册发现（Service / EndpointSlice / CoreDNS） |
| **服务治理** | `gateway` + 各域 `brpc`（限流、超时、重试、熔断等） |
| **缓存** | Redis |
| **数据库** | **PostgreSQL** |
| **消息队列** | Kafka |
...
### 8.1 Kubernetes 使用边界与服务治理基线

本项目默认以 **Kubernetes 原生服务发现** 作为生产基线；Kubernetes 在当前阶段仅承担编排与注册发现，不承担额外服务治理能力。

- **Kubernetes 使用范围**：`Deployment/StatefulSet` 编排 + `Service/EndpointSlice/CoreDNS` 注册发现。
- **入口边界**：客户端只经前置 `Nginx` 进入 `gateway`；业务域之间保持内部 `proto` 调用，不直接暴露对外 JSON 面。
- **服务发现真源**：`Service + EndpointSlice + CoreDNS`。业务方使用稳定服务名访问，不使用 Pod IP 作为契约。
- **治理边界**：连接、超时、重试、熔断、灰度等运行时治理由 `gateway` 与各域 `brpc` 承担；业务规则仍归各业务域。
- **当前不启用项**：Ingress、Service Mesh、NetworkPolicy 等额外治理能力暂不作为 v1 必需项。
- **可观测统一**：服务指标、网关指标、日志、追踪统一汇聚到 Prometheus / Grafana / ELK / Jaeger。
...
前端目标是在不改变业务语义的前提下，以最少操作成本完成“浏览 -> 理解 -> 跳转 -> 回流反馈”的主流程。终端层只负责展示与交互，不承载业务真相。

### 9.1 跨端统一约束

- 三端（Web、iOS、Android）共享同一套页面信息架构和核心任务路径
- 业务字段、错误语义、分页语义统一来源于 `docs/contracts/` 与 `services/gateway/docs/api.md`
- 客户端只依赖 `gateway` 的 JSON 契约，不直接依赖内部 `proto`
- 允许交互形态因平台差异适配，但不允许改写业务含义

### 9.2 交互效率原则

- 内容优先：首屏优先呈现可决策信息，减少非必要装饰
- 低认知负担：关键动作保持稳定位置与稳定文案
- 反馈及时：点击、加载、失败、重试都必须有可见反馈
- 动效克制：动效只服务状态变化和空间关系，不做炫技动画
- 渐进揭示：默认展示最关键信息，次级信息按需展开

### 9.3 状态模型总则

每个页面和核心组件都必须定义并覆盖以下状态：
...
状态触发条件、用户可见反馈和重试行为由前端文档统一定义，避免三端分裂实现。

### 9.4 与 Gateway 的交互边界

- 页面数据统一由 `gateway` 页面接口或资源接口提供
- `gateway` 负责 JSON 与内部 `proto` 映射，客户端只处理稳定 JSON
- 错误处理以 `code` 为准，`message` 仅用于展示
- 与页面聚合相关的编排语义以 `services/gateway/docs/pages.md` 和 `services/gateway/docs/api.md` 为准
